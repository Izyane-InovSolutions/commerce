...
export class PaymentsService {
  ...
  /**
   * Kept for the existing admin refund route's response shape: a RefundCase
   * is the business obligation now (see RefundCasesService), and this
   * surfaces its sole provider-attempt row so callers coded against a single
   * `Refund` keep working unchanged.
   */
  async refundSellerOrder(
    ...
    const refundCase = await this.refundCasesService.createCase({
      sellerOrderId,
    ...
    return this.soleRefundAttempt(refundCase.id);
  }
  ...
  private async soleRefundAttempt(refundCaseId: string): Promise<Refund> {
    const attempts = await this.prisma.refund.findMany({
      where: { refundCaseId },
      orderBy: { createdAt: 'asc' },
    });
    const attempt = attempts[0];
    if (!attempt)
      throw new NotFoundException('Refund case has no provider attempt');
    return attempt;
  }
}
